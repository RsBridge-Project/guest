...
struct inode {
	...
	unsigned long 	ib_enabled;
	bool 	ib_es_first;
	spinlock_t ib_es_lock;
	struct inode_extent_status ib_es[MAX_NR_IB_ES];
			       ...
			       struct file *filp)
{
	*kiocb = (struct kiocb) {
		...
		.ib_bpf_ino = kiocb_src->ib_bpf_ino,
		.scratch_buf = kiocb_src->scratch_buf,
	};
}
