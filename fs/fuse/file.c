			 ...
			 size_t count, int opcode)
{
	...
	if(opcode == FUSE_LOAD_BPF)
	{
		if(io->ib_bpf_ino > 0)
			{
				ia->read.in.bpf_ino = io->ib_bpf_ino;
				args->bpf_ino = io->ib_bpf_ino;
				printk("fuse_read_args_fill: bpf_ino %ld\n", io->ib_bpf_ino);
			      ...
			      fl_owner_t owner)
{
	...
	if(io->ib_bpf_ino)
	{
		fuse_read_args_fill(ia, file, pos, count, FUSE_LOAD_BPF);
	}
	else
	{
...
static ssize_t fuse_direct_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	if (!is_sync_kiocb(iocb) && iocb->ki_flags & IOCB_DIRECT) {
		...
		io.ib_bpf_ino = iocb->ib_bpf_ino;
		// io.ib_bpf_ino = iocb->ib_enable;
