...
static int __es_insert_extent(struct inode *inode, struct extent_status *newes)
{
	...
	while (*p) {
		...
		if (newes->es_lblk < es->es_lblk) {
			if (ext4_es_can_be_merged(newes, es)) {
				...
				if(inode->ib_enabled > 0)
					{
						spin_lock(&inode->ib_es_lock);
						if(inode->nr_ib_es >= MAX_NR_IB_ES)
						{
							printk("Error! the ib_es in the inode is not enough!\n");
						}
						else
						{
							inode->ib_es[inode->nr_ib_es].es_lblk = newes->es_lblk;
							inode->ib_es[inode->nr_ib_es].es_len = newes->es_len;
							inode->ib_es[inode->nr_ib_es].es_pblk  = newes->es_pblk & ~ES_MASK;
							inode->nr_ib_es += 1;
						}
	...
	es = ext4_es_alloc_extent(inode, newes->es_lblk, newes->es_len,
				  newes->es_pblk);
	if(inode->ib_enabled > 0 && newes->es_len!=0xFFFFFFFF)
	{	
		spin_lock(&inode->ib_es_lock);
		if(inode->nr_ib_es >= MAX_NR_IB_ES)
		{
