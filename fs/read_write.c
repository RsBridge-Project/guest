...
static ssize_t new_sync_read_ib(struct file *filp, char __user *buf, size_t len, loff_t *ppos, 
			unsigned long ib_bpf_ino, char __user *scratch_buf)
{
	struct kiocb kiocb;
	struct iov_iter iter;
	struct page *page;
	...
	init_sync_kiocb(&kiocb, filp);
	kiocb.ki_pos = (ppos ? *ppos : 0);
	kiocb.ib_bpf_ino = ib_bpf_ino;
	kiocb.scratch_buf = NULL;
	if (get_user_pages_fast((unsigned long)scratch_buf, 1, FOLL_WRITE, &page)!=1){
		printk("new_sync_read_ib: failed to get user pages\n");
		return -EFAULT;
	}else{
		kiocb.scratch_buf = page;
	}
...
static ssize_t new_sync_write_ib(struct file *filp, const char __user *buf, size_t len, loff_t *ppos, unsigned int ib_enable)
{
	struct kiocb kiocb;
	struct iov_iter iter;
	ssize_t ret;
...
ssize_t vfs_write_ib(struct file *file, const char __user *buf, size_t count, loff_t *pos, unsigned int ib_enable)
{
	...
	if (file->f_op->write)
		...
		ret = new_sync_write_ib(file, buf, count, pos, ib_enable);
	else
		ret = -EINVAL;
	if (ret > 0) {
		fsnotify_modify(file);
...
ssize_t vfs_read_ib(struct file *file, char __user *data_buf, size_t count,
		    loff_t *pos, unsigned long ib_bpf_ino,
		    char __user *scratch_buf)
{
	...
	if (ib_bpf_ino > 0) {
		inode->ib_enabled = ib_bpf_ino;
	}

	if (file->f_op->read_iter)
		// use scratch_buf to pass and get result instead of inode.query
		ret = new_sync_read_ib(file, data_buf, count, pos, ib_bpf_ino, scratch_buf);
	else
		ret = -EINVAL;
	if (ret > 0) {
...
ssize_t ksys_read_ib(unsigned int fd, char __user *data_buf, size_t count,
		     loff_t pos, unsigned long ib_bpf_ino,
		     char __user *scratch_buf)
{
	struct fd f;
	...
	if (f.file) {
		...
		if (f.file->f_mode & FMODE_PREAD)
			ret = vfs_read_ib(f.file, data_buf, count, &pos,
					  ib_bpf_ino, scratch_buf);
		fdput(f);
	}
...
SYSCALL_DEFINE6(read_ib, unsigned int, fd, char __user *, buf, size_t, count, loff_t, pos, unsigned long, ib_bpf_ino, char __user *,scratch_buf)
{
	return ksys_read_ib(fd, buf, count, pos, ib_bpf_ino, scratch_buf);
}
